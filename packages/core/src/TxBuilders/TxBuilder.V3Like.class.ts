...
import {
  DatumBuilderV3Like,
  TDatumBuilderMintV3LikePoolArgs,
} from "../DatumBuilders/DatumBuilder.V3Like.class.js";
 ...
 */
export class TxBuilderV3Like extends TxBuilderAbstractV3 {
  contractVersion: EContractVersion = EContractVersion.V3;
  datumBuilder: DatumBuilderV3Like;
   ...
   * @returns {TxBuilderV3Like}
   */
  public enableTracing(enable: boolean): TxBuilderV3Like {
    this.tracing = enable;
    return this;
  }
  ...
  ): Promise<TDatumBuilderMintV3LikePoolArgs> {
    return {
      assetA: sortedAssets[0],
      assetB: sortedAssets[1],
      fees: args.fees!,
      marketOpen: args.marketTimings || 0n,
   ...
   */
  async mintPool(
    args: TMintV3LikePoolConfigArgs,
  ): Promise<IComposedTx<BlazeTx, Core.Transaction>> {
    const config = new MintV3LikePoolConfig(args);
    const { assetA, assetB, ownerAddress, referralFee, donateToTreasury } =
      config.buildArgs();
    ...
    const newPoolIdent = DatumBuilderV3Like.computePoolId(seedUtxo);

    const nftAssetName = DatumBuilderV3Like.computePoolNftName(newPoolIdent);
    const poolNftAssetIdHex = `${poolPolicyId + nftAssetName}`;

    const refAssetName = DatumBuilderV3Like.computePoolRefName(newPoolIdent);
    const poolRefAssetIdHex = `${poolPolicyId + refAssetName}`;

    const poolLqAssetName = DatumBuilderV3Like.computePoolLqName(newPoolIdent);
    ...
    const {
      ...
      await this.buildMintPoolDatumArgs(sortedAssets, seedUtxo, config),
    );
    ...
    if (donateToTreasury) {
      const realTreasuryAddress = DatumBuilderV3Like.getTreasuryAddress(
        settingsDatum,
        this.network === "mainnet"
          ? Core.NetworkId.Mainnet
          : Core.NetworkId.Testnet,
  ...
  async cancel(
    ...
    try {
      ...
      return this.handleOtherOrderTypeCancellation(cancelArgs);
    }
    ...
    const signerKey = DatumBuilderV3Like.getSignerKeyFromDatum(
  ...
  public async getUtxosForPoolMint(
    ...
    for (const selinp of chosenUtxos.selectedInputs) {
      console.log(
        `Selected UTXO: ${selinp.input().transactionId()}#${selinp.input().index()}`,
      );
    }
