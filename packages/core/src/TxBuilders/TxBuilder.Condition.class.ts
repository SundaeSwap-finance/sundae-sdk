...
import { TxBuilderAbstractCondition } from "src/Abstracts/TxBuilderAbstract.Condition";
import {
  CancelConfig,
  DepositConfig,
  SwapConfig,
...
export class TxBuilderCondition implements TxBuilderAbstractCondition {
  contractVersion: EContractVersion = EContractVersion.Condition;
  datumBuilder: DatumBuilderCondition;
  ...
  constructor(
    ...
    this.datumBuilder = new DatumBuilderCondition(network);
  }
   ...
   * @returns {TxBuilderCondition}
   */
  public enableTracing(enable: boolean): TxBuilderCondition {
    this.tracing = enable;
    return this;
  }
  ...
  async cancel(
    ...
    /**
     * If we can properly deserialize the order datum using a Condition type, then it's a Condition order.
    ...
    const signerKey = DatumBuilderCondition.getSignerKeyFromDatum(
  ...
  async zap(
    ...
    const { inline } = this.datumBuilder.buildSwapDatum({
      ...
      destinationAddress: {
        ...
        /**
         * @TODO
         * Assuming that we can use inline datums in the Condition Scooper,
   ...
   * @throws {Error} Throws an error if the transaction fails to build or submit.
   */
  protected async mintPoolGeneric(
    mintPoolArgs: IMintConditionPoolConfigArgs,
  ): Promise<IComposedTx<BlazeTx, Core.Transaction>> {
    const {
      assetA,
